/*
 * Pass or Fail Problem Code: PASSORFAIL
...
namespace PassOrFail
{
    class MainClass
    {
        ...
        {
            int n, x, p;
            Console.Write($"Enter the passmark");
            p = Console.Read();

            Console.Write($"Enter the wrong answers");
            x = Console.Read();

            Console.Write($"Enter the total attempted");
            n = Console.Read();

            CalculatePassOrFail(n, x, p);
            
        }
        static void CalculatePassOrFail(int attemp, int wrong, int pass)
        {
            int correct = (attemp - wrong) * 2;
